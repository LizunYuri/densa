...
const MaterialsList = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/materials/materials/');
      if (response.ok) {
        const result = await response.json();
            
            
        if (Array.isArray(result.materials)) {
          setGetRecord(result.materials);
        } else {
  ...
  const deleteFetch = async (id) => {
    try{
      const response = await fetch (`materials/delete/materials/${id}/`,
        {
          method : 'DELETE',
          headers : {
              ...
              <div className="dashboard_table_body">
                ...
                <div className={`upload_form ${editModalVisible ? 'dashboard_visible' : 'dashboard_not_visible'}`}>
                  <div className="upload_form_body">
                    ...
                    {editModalVisible && (
                      <MaterialsUpdate recordId={editRecordInformation.id} onRecordUpdated={fetchData}  />
                    )}
                  </div>
                </div>
