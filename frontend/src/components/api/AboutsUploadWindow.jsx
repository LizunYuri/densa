...
import AboutUpdateRecord from './AboutUpdateRecord';
import AboutAddRecotd from './AboutAddRecotd';


const AboutsUploadWindow = () => {
  ...
  const deleteFetch = async (id) => {
    try{
      const response = await fetch (`/about/about-delete/about/${id}/`,
        {
          method : 'DELETE',
          headers : {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/about/about/');
        ...
        if (Array.isArray(result.abouts)) {
          setGetRecordAbout(result.abouts);
        } else {
  ...
  return (
    ...
    {/* {isAuthenticated ? ( */}
      <>
        <h2>Раздел "О нас"</h2>
        <p>Информация блока важна для сео продвижения страницы. При отсутсвии записей в таблице данный блок не отображается на сайте.</p>
        ...
        <div className={`upload_form ${editModalVisible ? 'dashboard_visible' : 'dashboard_not_visible'}`}>
          <div className="upload_form_body">
            <FaWindowClose onClick={notEditModal} className='closed_btn_modal' />
              {editModalVisible && (
                <AboutUpdateRecord recordId={editRecordInformation.id} onRecordUpdated={fetchData}/>
              )}
          </div>
        </div>
      </>
...
export default AboutsUploadWindow;